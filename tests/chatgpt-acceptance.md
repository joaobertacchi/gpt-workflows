...
- Source verification: passed (`python3 scripts/test_flow.py`, 30/30)
- Installed-cache verification: passed (`diff -rq`, no differences after final acceptance evidence update)
- Auxiliary Codex loading: pending for 0.3.2
...
Quit and reopen ChatGPT after installing version `0.3.2`. On the ChatGPT homepage, switch from **Chat** to **Work**. For each scenario, start a new Work conversation, type `@`, and select **Documentar Reunião** from the menu. Do not use the regular Chat surface or manually typed mention text as acceptance evidence.
